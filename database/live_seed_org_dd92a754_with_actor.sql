...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE id = 'dd92a754-e8c7-4080-951c-b0899891f35e'::uuid
    ) THEN
        RAISE EXCEPTION 'Target org % does not exist', 'dd92a754-e8c7-4080-951c-b0899891f35e';
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM auth.roles
        WHERE id = 'ad41fba5-e4cf-40b6-8455-b0dfae0e2457'::uuid
          AND org_id = 'dd92a754-e8c7-4080-951c-b0899891f35e'::uuid
    ) THEN
        RAISE EXCEPTION 'Role % does not exist in org %', 'ad41fba5-e4cf-40b6-8455-b0dfae0e2457', 'dd92a754-e8c7-4080-951c-b0899891f35e';
    END IF;
END $$;

-- Live-safe seed for org:   dd92a754-e8c7-4080-951c-b0899891f35e
-- Acting account id:        d3500075-a900-4474-bb3d-8dc82fc6a138
--
-- Scope:
-- - verifies the target org and role exist
-- - upserts the provided Cognito account row
...
WITH target_context AS (
    SELECT
        'dd92a754-e8c7-4080-951c-b0899891f35e'::uuid AS org_id,
        'd3500075-a900-4474-bb3d-8dc82fc6a138'::uuid AS actor_id,
        'yeojiahao89@gmail.com'::text AS email,
        'silver1713'::text AS username,
        'cognito'::text AS auth_provider,
        'd428f4b8-2081-7051-190c-8519610dcf58'::text AS cognito_sub,
        'ad41fba5-e4cf-40b6-8455-b0dfae0e2457'::uuid AS role_id,
        TIMESTAMP '2026-03-30 14:41:22.325804' AS created_at,
        TIMESTAMP '2026-03-30 14:41:22.325804' AS updated_at
),
...
WHERE EXISTS (
    ...
    WHERE a.id = vc.actor_id
      AND a.org_id = vc.org_id
)
  AND NOT EXISTS (
    SELECT 1
